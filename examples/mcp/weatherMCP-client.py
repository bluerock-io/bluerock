...
class MCPClient:
    ...
    def __init__(self, llm_host, model: str = "llama3.1:latest"):
        self.console = Console()
        self.exit_stack = AsyncExitStack()
        self.model = model
        self.ollama = ollama.AsyncClient(host=llm_host)
        ...
        # Check for tool calls
            ...
            for tc in resp.message.tool_calls:
                ...
                resp = await self.ollama.chat(
                    model=self.model, messages=messages, tools=tools_meta, options={"max_tokens": 500}
                )
                final.append(resp.message.content)
        return "".join(final)
...
async def main():
    ...
    p.add_argument("--llm-host", required=True)
    p.add_argument("--model", default="llama3.2:3b")
    args = p.parse_args()
    print(args.llm_host)
    llm_url = f"http://{args.llm_host}:11434"
    client = MCPClient(llm_url, model=args.model)
    try:
